...
// 内置插件：随应用打进 resources/runtime/node_modules/@dsh-desktop/
...
const PATCH_MARKER = '# Auto-generated by dsh-desktop (builtin @dsh-desktop plugins)'
...
/**
 * 自动注入内置插件到 profile 的 cordis.patch.yml。
 * 路径基于 process.resourcesPath（portable 解压目录 / 安装版 resources / 开发 win-unpacked 通用），
 * 并以 file:// URL 形式书写（dsh 的 ESM loader 不接受 Windows 盘符绝对路径，否则 ERR_UNSUPPORTED_ESM_URL_SCHEME）。
 * 幂等：文件已含 marker 则跳过重写。
 */
function ensurePluginPatch() {
  try {
    const profileDir = path.join(app.getPath('userData'), 'profiles', 'web')
    fs.mkdirSync(profileDir, { recursive: true })
    const patchFile = path.join(profileDir, 'cordis.patch.yml')
    if (fs.existsSync(patchFile)) {
      const existing = fs.readFileSync(patchFile, 'utf8')
      if (existing.includes(PATCH_MARKER)) return true
    }
    const pluginsRoot = path.join(process.resourcesPath, 'runtime', 'node_modules', '@dsh-desktop')
    const available = BUILTIN_PLUGINS.filter((p) =>
      fs.existsSync(path.join(pluginsRoot, p, 'lib', 'index.js')),
    )
    if (available.length === 0) {
      log('[dsh-desktop] 未发现内置插件（跳过自动注入）')
      return false
    }
    const entries = available.map((p) => {
      const entry = path.join(pluginsRoot, p, 'lib', 'index.js')
      return `    - id: ${p}\n      name: '${pathToFileURL(entry).href}'`
    })
    const content = `${PATCH_MARKER}\n- insert:\n${entries.join('\n')}\n`
    fs.writeFileSync(patchFile, content)
    log('[dsh-desktop] 已自动注入内置插件 patch:', available.join(','))
    return true
  } catch (err) {
    log('[dsh-desktop] 注入插件 patch 失败:', err && err.message)
    return false
  }
}
